...
const countryPoints: Record<string, [number, number]> = {
  ...
  DE: [375, 122], FR: [360, 138], NG: [365, 220], ZA: [400, 292], AE: [438, 183],
...
function WorldMap({ countries }: { countries: CountItem[] }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-[#00ffd5]/20 bg-[#031118] p-3">
      <svg viewBox="0 0 720 340" className="h-[280px] w-full" role="img" aria-label="Live visitor country map">
        <defs>
        ...
        {countries.map((item) => {
          ...
          const radius = 5 + (item.count / max) * 12;
          return (
            <g key={item.name} filter="url(#admin-glow)">
              <circle cx={point[0]} cy={point[1]} r={radius + 7} fill="#00ffd5" opacity=".08" />
              <circle cx={point[0]} cy={point[1]} r={radius} fill="#00ffd5" opacity=".9" />
    ...
    <main className="min-h-screen bg-[#02090d] px-3 py-5 font-mono-cyber text-[#e0f7f6] sm:px-6 lg:px-10">
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        <section className="grid gap-5 lg:grid-cols-3">
          ...
          <div className="rounded-2xl border border-[#00ffd5]/15 bg-[#041820]/90 p-4"><h2 className="font-display text-lg font-bold">Data status</h2><div className="mt-4 space-y-3 text-xs text-white/60"><p><span className="text-[#00ffd5]">Captured since:</span> {new Date(metrics.capturedSince).toLocaleString()}</p><p><span className="text-[#00ffd5]">Last refresh:</span> {new Date(metrics.generatedAt).toLocaleTimeString()}</p><p><span className="text-[#00ffd5]">Retention:</span> {metrics.retention}</p><p className="rounded-xl border border-[#ffd166]/20 bg-[#ffd166]/5 p-3 text-[#ffe7a5]">For historical reporting after a restart, add a GA4 measurement ID or connect durable storage.</p></div></div>
        </section>

        <section className="rounded-2xl border border-[#00ffd5]/15 bg-[#041820]/90 p-4"><div className="mb-3 flex items-center justify-between"><div><h2 className="font-display text-lg font-bold">Recent signal feed</h2><p className="text-[11px] text-white/45">Latest events captured by the service</p></div><span className="text-[10px] uppercase tracking-[0.18em] text-[#00ffd5]/60">Auto refresh 15s</span></div><div className="overflow-x-auto"><table className="w-full min-w-[620px] text-left text-xs"><thead className="text-[10px] uppercase tracking-[0.16em] text-white/35"><tr><th className="pb-3">Event</th><th className="pb-3">Country</th><th className="pb-3">Platform</th><th className="pb-3">Format</th><th className="pb-3">Time</th><th className="pb-3">Status</th></tr></thead><tbody>{metrics.recentActivity.map((item) => <tr key={item.id} className="border-t border-white/5"><td className="py-3 text-white/75">{item.name.replaceAll('_', ' ')}</td><td className="py-3 text-[#00ffd5]">{item.country}</td><td className="py-3 capitalize text-white/60">{item.platform || '—'}</td><td className="py-3 uppercase text-white/60">{item.format || '—'}</td><td className="py-3 text-white/45">{formatTime(item.timestamp)}</td><td className={`py-3 ${item.success === false ? 'text-red-300' : 'text-[#00e599]'}`}>{item.success === false ? 'FAILED' : 'CAPTURED'}</td></tr>)}</tbody></table>{!metrics.recentActivity.length && <p className="py-8 text-center text-sm text-white/40">Waiting for the first signal.</p>}</div></section>
      </div>
    </main>
  );
};
